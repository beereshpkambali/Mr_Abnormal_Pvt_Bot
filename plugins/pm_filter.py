...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("😁 𝗛𝗲𝘆 𝗙𝗿𝗶𝗲𝗻𝗱,𝗣𝗹𝗲𝗮𝘀𝗲 𝗦𝗲𝗮𝗿𝗰𝗵 𝗬𝗼𝘂𝗿𝘀𝗲𝗹𝗳. /n /nಲೇ ತಮ್ಮಾ😄.. ಇದು ನಿನ್ನ Request ಅಲ್ಲಲೇ.", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
    ...
    if query.data.startswith("file"):
        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                ...
                await query.answer('Check PM, I have sent files in pm. \n \n(ನಿನ್ನ ವೈಯಕ್ತಿಕ ಖಾತೆಗೆ ಮೂವಿಯನ್ನು ಕಳುಹಿಸಿದ್ದೇನೆ, ನೋಡಿ)', show_alert=True)
